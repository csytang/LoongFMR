...
import loongpluginfmrtool.toolbox.bunch.BunchServer.FindNeighbor;
...
int[] getMoreWork(int [] lastWorkVector)
{
   ...
   try
   {
      byte [] so = loongpluginfmrtool.toolbox.bunch.util.BunchUtilities.toByteArray(iMgr);
      byte [] rtdObj = cliCallback.callFromServerWithObj("GET_NEXT_VECTOR",so);
      iMgr = (IterationManager)loongpluginfmrtool.toolbox.bunch.util.BunchUtilities.fromByteArray(rtdObj);
