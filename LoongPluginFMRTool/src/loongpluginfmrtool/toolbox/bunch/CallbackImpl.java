...
import loongpluginfmrtool.toolbox.bunch.WorkFinishedEvent;
import loongpluginfmrtool.toolbox.bunch.WorkRequestEvent;
...
public class CallbackImpl extends PortableRemoteObject implements Callback{
  ...
  public loongpluginfmrtool.toolbox.bunch.LoadBalancer.Manager lbManager = null;
  ...
  public byte[] callFromServerWithObj(String input, byte[]so)
  {
    ...
    if(input.equals(GET_NEXT_VECTOR))
    {
      try
      {
         ...
         * of message
         */
        IterationManager im = (IterationManager)loongpluginfmrtool.toolbox.bunch.util.BunchUtilities.fromByteArray(so);
         ...
         */
        byte [] soRet = loongpluginfmrtool.toolbox.bunch.util.BunchUtilities.toByteArray(im);
        return soRet;
      }
      catch(Exception ex)
      {
